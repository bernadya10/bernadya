...
export function RoleEditorEnterprise({userRole}:{userRole:string}){
 ...
 async function save(){if(userRole!=='Super Admin'||!selected)return;for(const code of all){const has=perms.has(code);const {data:existing}=await supabase.from('hris_role_permissions').select('id').eq('role_name',selected).eq('permission_code',code).maybeSingle();if(has&&!existing)await supabase.from('hris_role_permissions').insert({role_name:selected,permission_code:code});if(!has&&existing)await supabase.from('hris_role_permissions').delete().eq('id',existing.id)}alert('Permission tersimpan.');}
...
export function ApprovalCenter(){const {t}=useTranslation();
 ...
 return <><Heading title="Approval Command Center" desc="Satu inbox untuk approval HR, payroll dan seluruh pengajuan Employee Self Service. Keputusan diproses server-side dan dicatat ke audit trail."/><div className="branch-nav">{[['all','Semua'],['leave','Cuti'],['overtime','Lembur'],['payroll','Payroll'],['ess_leave','Cuti ESS'],['ess_overtime','Lembur ESS'],['ess_attendance','Absensi ESS'],['ess_profile','Profil ESS']].map(([k,v])=><button key={k} className={tab===k?'active':''} onClick={()=>setTab(k as any)}><AdminIcon name={k} size={15}/><span>{v}</span></button>)}</div>{msg&&<div className="alert">{msg}</div>}<div className="toolbar-panel"><label>Cari approval<input className="filter" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Modul, requester, ID..."/></label><div className="stat-inline"><b>{visible.length}</b><span>{t('pending')}</span></div><button className="secondary" onClick={load}>{t('refresh')}</button></div><div className="panel table-panel"><div className="table-wrap"><table><thead><tr><th>Modul</th><th>Record</th><th>Requester</th><th>Approver Role</th><th>Dibuat</th><th>{t("action")}</th></tr></thead><tbody>{loading?<Empty cols={6}/>:visible.length?visible.map(r=><tr key={r.id}><td><b>{label(r.modul)}</b></td><td>{String(r.record_id).slice(0,12)}</td><td>{r.requester_email||'—'}</td><td><Status v={r.approver_role||'Super Admin'}/></td><td>{r.created_at?new Date(r.created_at).toLocaleString('id-ID'):'—'}</td><td><div className="row-actions"><button className="primary" onClick={()=>decide(r,'Disetujui')}>{t('approve')}</button><button className="secondary" onClick={()=>decide(r,'Ditolak')}>{t('reject')}</button></div></td></tr>):<Empty cols={6}/>}</tbody></table></div></div></>}
