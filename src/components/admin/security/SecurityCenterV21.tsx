...
export default function SecurityCenterV21(){
 ...
 async function logTest(){if(!canManage)return;const {error}=await supabase.rpc('hris_v21_security_event',{p_event_type:'SECURITY_CENTER_VIEW',p_metadata:{source:'Pusat Keamanan'}});if(error)setMsg(error.message);else{setMsg('Security event tercatat.');load()}}
 return <div className="module-page">
  ...
  <div className="ess-kpis">
   ...
   <div className="ess-kpi"><span>Permission</span><strong>{perms.length}</strong><small>{perms.includes('*')?'Akses Penuh':'Akses Terperinci'}</small></div>
   <div className="ess-kpi"><span>Event Keamanan</span><strong>{events.length}</strong><small>100 event terakhir</small></div>
