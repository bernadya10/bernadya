...
export default function PortalKaryawan({onLogout}:{onLogout?:()=>void}){
 ...
 return <div className="employee-portal"><header className="employee-topbar"><div className="employee-brand"><div className="employee-logo"><img src={moonLogo} alt="Project by Tirta" /></div><div><strong>Project by Tirta</strong><small>Employee Self Service</small></div></div><div className="employee-user"><div className="notification-dot">{stats.unread}</div><div><b>{employee.nama}</b><small>{employee.jabatan||t('employee')}</small></div><button className="portal-logout" onClick={logout}>Keluar</button></div></header>
 <main className="employee-page"><div className="employee-heading"><div><span className="portal-eyebrow">EMPLOYEE SELF SERVICE · V12</span><h1>{tab==='home'?t('welcome') + ', '+employee.nama:tabs.find(x=>x[0]===tab)?.[1]}</h1><p>Absensi aman, layanan HR, payroll, dan pengajuan dalam satu portal.</p></div><div className="date-chip">{new Intl.DateTimeFormat('id-ID',{dateStyle:'full'}).format(new Date())}</div></div>
